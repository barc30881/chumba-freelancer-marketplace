...
(function(win, $) {
    ...
    const handlePriceRange = function() {
        ...
        minPrice.on('change', function() {
            ...
            if ($(this).val() < 0) {
                ...
                progress.css({
                    'left': ($(this).val() / rangeInput.eq(0).attr('max')) * 100 + "%"
                });
            } else if ($(this).val() >= maxValue - priceGap) {
                $(this).val(maxValue - priceGap)
                rangeInput.eq(0).val(maxValue - priceGap)
                progress.css({
                    'left': ((maxValue - priceGap) / rangeInput.eq(0).attr('max')) * 100 + "%"
                });
            }
        })
        ...
        maxPrice.on('change', function() {
            ...
            if (parseInt($(this).val()) > rangeInput.eq(1).attr('max')) {
                ...
                progress.css({
                    'right': 100 - ($(this).val() / rangeInput.eq(1).attr('max')) * 100 + "%"
                });
            } else if (parseInt($(this).val()) <= minValue + priceGap) {
                $(this).val(minValue + priceGap)
                rangeInput.eq(1).val(minValue + priceGap)
                progress.css({
                    'right': 100 - ((minValue + priceGap) / rangeInput.eq(1).attr('max')) * 100 + "%"
                });
            }
        })
    ...
    const handleRadius = function() {
        ...
        rangeInput.on('input', function() {
            ...
            progress.css({
                'right': 100 - (radiusValue / rangeInput.attr('max')) * 100 + "%"
            });
